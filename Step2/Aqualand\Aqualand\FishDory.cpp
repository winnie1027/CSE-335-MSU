...
*/
CFishDory::CFishDory(CAquarium* aquarium) : CItem(aquarium)
{
    mFishImage = unique_ptr<Bitmap>(Bitmap::FromFile(FishDoryImageName.c_str()));
    if (mFishImage->GetLastStatus() != Ok)
    {
        wstring msg(L"Failed to open ");
        msg += FishDoryImageName;
 ...
 */
void CFishDory::Draw(Gdiplus::Graphics* graphics)
{
 ...
 * \return true if hit.
 */
bool CFishDory::HitTest(int x, int y)
{
    double wid = mFishImage->GetWidth();
    double hit = mFishImage->GetHeight();
