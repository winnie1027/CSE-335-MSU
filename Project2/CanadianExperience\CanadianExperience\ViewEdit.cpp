...
BEGIN_MESSAGE_MAP(CViewEdit, CScrollView)
    ...
    ON_WM_LBUTTONDOWN()
    ON_WM_MOUSEMOVE()
    ON_COMMAND(ID_MACHINE_MACHINEONE, &CViewEdit::OnMachineMachineone)
    ON_COMMAND(ID_MACHINE_MACHINETWO, &CViewEdit::OnMachineMachinetwo)
END_MESSAGE_MAP()
...
* Turn on machine one.
*/
void CViewEdit::OnMachineMachineone()
{
    GetPicture()->GetMachineOne()->TurnOnMachine();
}

/**
* Turn on machine two.
*/
void CViewEdit::OnMachineMachinetwo()
{
    GetPicture()->GetMachineTwo()->TurnOnMachine();
}
