...
/// Maximum possible frame number default
...
BOOL CMachineDemoDlg::OnInitDialog()
{
    ...
    mTimeSlider.SetRange(0, MaxFrame, TRUE);
	...
	if (mInitialMachine > 0)
	{
		mMachineView.SetMachineNumber(mInitialMachine);
	}
...
*/
void CMachineDemoDlg::SetFrameNum(int frame)
{
	if (frame < 0)
	{
		mFrameNum = 0;
...
/**
 * Set the initial machine number to use
 * @param num Number to use
*/
void CMachineDemoDlg::SetMachineNum(int num)
{
	mInitialMachine = num;
}