...
void CCity::XmlTile(const std::shared_ptr<CXmlNode> &node)
{
    ...
    {
        tile = make_shared<CTileRocketPad>(this);
    }
...
*  @return Generated report of type CCityReport
*/
std::shared_ptr<CCityReport> CCity::GenerateCityReport()
{
    auto report = make_shared<CCityReport>(this);

    for (auto item : mTiles)
    {
        auto memberReport = make_shared<CMemberReport>(item);
